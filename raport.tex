...
Le flux RTSP de chaque caméra est transcodé par FFmpeg en segments fMP4 servis via nginx. La latence est de 3 à 5 secondes. Ce protocole est utilisé pour l'enregistrement continu et comme fallback universel.
...
\hline
Streaming & MediaMTX (RTSP/WebRTC) & Serveur de flux local & Redémarrage Systemd \\
\hline
...
\begin{description}[leftmargin=2.5cm, style=nextline]
    ...
    \item[\texttt{motion\_events}]
    Événements de mouvement détectés. Champs : \texttt{id}, \texttt{device\_id} (FK \texttt{camera\_nodes}), \texttt{motion}, \texttt{offline\_recording} (booléen), \texttt{recording\_path}, \texttt{detected\_at}, \texttt{created\_at}.
...
Le schéma garantit l'intégrité des données via des clés étrangères entre \texttt{alerts} et \texttt{cameras}, entre \texttt{alerts} et \texttt{users} (acquittement), et entre \texttt{motion\_events} et \texttt{camera\_nodes}.
