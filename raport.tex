...
Par souci de transparence, je précise ici comment j'ai utilisé les outils d'intelligence artificielle générative dans la préparation des livrables associés à ce TFE.

\paragraph{Les outils utilisés}
Je me suis appuyé sur plusieurs outils, chacun pour un usage distinct :
\begin{itemize}[label=\textbullet]
    \item \textbf{Claude Code (ligne de commande) :} assistance au développement du système SENTYS --- débogage, révision de code, cohérence architecturale.
    \item \textbf{Claude (application de bureau) :} partenaire de réflexion sur les choix techniques et méthodologiques, et accompagnement lors de la rédaction de ce rapport (mise en forme \LaTeX{}, reformulation, structuration).
    \item \textbf{ChatGPT (OpenAI) :} consultation ponctuelle pour des questions techniques ou des doutes lors du développement.
    \item \textbf{Gemini (Google) :} recherches d'informations complémentaires et vérification de concepts.
\end{itemize}


\paragraph{Comment je m'en suis servi}
Je ne suis jamais parti d'une page blanche confiée à une IA. La base de travail était toujours personnelle : mes notes, mes décisions techniques déjà prises, les échanges avec mon rapporteur. L'IA est intervenue ensuite pour regrouper cette matière, la reformuler ou la corriger, dans l'objectif d'arriver à un résultat plus soigné et plus cohérent.

\paragraph{Validation et responsabilité}
Tout ce qu'un outil d'IA a produit --- du code intégré au système comme des paragraphes de ce rapport --- est passé par ma relecture et a été validé par moi avant d'être conservé. Je reste le seul responsable du contenu livré. Les analyses, les choix techniques et les positions exprimées dans ce document viennent de mon propre travail et de ma propre réflexion.
...
Le schéma garantit l'intégrité des données via des clés étrangères entre \texttt{alerts} et \texttt{cameras}, entre \texttt{alerts} et \texttt{users} (acquittement), entre \texttt{camera\_node\_motion\_events} et \texttt{camera\_nodes}, et entre \texttt{push\_subscriptions} et \texttt{users}. La table \texttt{audit\_logs} ne référence intentionnellement aucune entité par clé étrangère afin de garantir la permanence des traces même après suppression d'un utilisateur. La table \texttt{app\_settings} est protégée par une contrainte \texttt{CHECK (id = 1)} garantissant l'unicité de la configuration.
...
\centering
\small
\begin{tabular}{|p{2.8cm}|p{3.2cm}|p{5.5cm}|p{3.2cm}|}
\hline
